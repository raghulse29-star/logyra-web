...
export default function PricingSection() {
  return (
    <section id="pricing" className="bg-[var(--color-grey-section)] py-12 md:py-16 lg:py-20">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
        ...
        <div className="mb-14">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-bold text-[var(--color-fg)] mb-4">
            Choose your access,{' '}
            <span className="text-[var(--color-primary)]">start learning today.</span>
          </h2>
          <p className="text-[var(--color-fg-subtle)] max-w-xl leading-relaxed">
            Unlock institutional-grade F&amp;O research and pre-market briefs. All plans include private Telegram access.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 border border-black/[0.08] rounded-xl overflow-hidden bg-white shadow-[0_4px_24px_rgba(10,10,10,0.06)]">
          {plans.map((plan, i) => (
            <motion.div
              key={plan.num}
              ...
              className={`relative flex flex-col p-5 sm:p-7 md:p-8 ${
                plan.featured
                  ...
                  : 'bg-white'
              } ${i < plans.length - 1 ? 'md:border-r border-b md:border-b-0 border-black/[0.08]' : ''}`}
            >
              {plan.badge && (
              ...
              <div className={`text-[11px] font-medium tracking-[0.12em] uppercase mb-5 text-[var(--color-fg-subtle)] ${plan.badge ? 'mt-5' : ''}`}>
                {plan.num} — {plan.label}
              </div>

              <div className="text-5xl font-bold text-[var(--color-fg)] mb-1">₹{plan.price}</div>
              <div className="text-xs text-[var(--color-fg-subtle)] mb-6">Incl. GST · {plan.duration}</div>

              {plan.savings ? (
                <div className="self-start text-[9px] font-bold tracking-[0.12em] uppercase px-3 py-1.5 mb-6 border border-[var(--color-primary)]/30 text-[var(--color-primary)] bg-[var(--color-primary-tint)] rounded-sm">
                  {plan.savings}
                </div>
              ) : (
                <div className="mb-6" />
              ...
              <ul className="space-y-0 mb-8 flex-1">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2 text-sm py-3 border-b border-black/[0.07] text-[var(--color-fg-muted)]">
              ...
              <Link
                ...
                className={`block w-full py-3.5 text-center text-[11px] font-bold tracking-[0.15em] uppercase rounded-lg transition-all duration-200 ${
                  plan.featured
                    ...
                    : 'border border-black/15 text-[var(--color-fg-muted)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]'
                }`}
              >
                Subscribe →
              </Link>
        ...
        <p className="text-center text-[var(--color-fg-subtle)] text-xs mt-6 tracking-wide">
