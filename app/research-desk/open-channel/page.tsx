    ...
    <main className="font-sans bg-[var(--color-bg)]">
      ...
      <div className="bg-[var(--color-bg-section)] border-b border-[var(--color-border)] pt-24 sm:pt-28">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-3 text-xs text-[var(--color-fg-subtle)] flex items-center gap-2">
          ...
          <span className="text-[var(--color-fg-subtle)]">›</span>
          <span className="text-[var(--color-fg-muted)]">Open Channel</span>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div>
              ...
              <motion.h1
                ...
                className="text-4xl md:text-5xl lg:text-6xl font-bold text-[var(--color-fg)] leading-[1.08] mb-5"
              >
                The market,<br />
                read every day.<br />
              ...
              >
                <button
                  type="button"
                  onClick={openChannel}
                  className="inline-flex items-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white text-[13px] font-bold tracking-[0.06em] uppercase px-6 py-3.5 rounded-lg transition-colors cursor-pointer"
                >
                ...
                <a
                  href="#whats-inside"
                  className="inline-flex items-center gap-2 border border-[var(--color-primary)] hover:bg-[var(--color-primary-tint)] text-[var(--color-primary)] text-[13px] font-bold tracking-[0.06em] uppercase px-6 py-3.5 rounded-lg transition-colors"
                >
                  See What&apos;s Inside
                </a>
              </motion.div>
              <p className="text-xs text-[var(--color-fg-subtle)]">₹0 · No credit card. No signup. Open to all.</p>
            </div>
            ...
            <motion.div
              ...
              className="relative"
            >
              <div className="bg-white rounded-2xl border border-[var(--color-border)] overflow-hidden shadow-[0_4px_24px_rgba(10,10,10,0.06)]">
                <div className="bg-[var(--color-beige-lighter)] px-5 py-3.5 flex items-center gap-3 border-b border-[var(--color-border)]">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-accent)] flex items-center justify-center font-bold text-white">L</div>
                  <div>
                    <div className="text-[var(--color-fg)] text-sm font-semibold">Logyra Intelligence</div>
                    <div className="text-[var(--color-fg-subtle)] text-xs">@logyra_insights</div>
                  </div>
                ...
                <div className="p-5 space-y-3">
                  {sampleMessages.map((msg) => (
                    <div key={msg.label} className="bg-[var(--color-beige-light)] rounded-xl rounded-bl-sm p-3.5">
                      ...
                      <div className="text-[var(--color-fg-subtle)] text-[10px] mt-1.5 text-right">{msg.time}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ── What's Inside ── */}
      <section id="whats-inside" className="bg-white py-14 md:py-20 lg:py-28">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {whatsInside.map((item, i) => (
              <motion.div
                ...
                className="p-7 bg-white border border-[var(--color-border)] rounded-xl hover:border-[var(--color-primary)]/30 shadow-[0_4px_24px_rgba(10,10,10,0.06)] transition-all duration-300"
              >
                <div className="w-11 h-11 rounded-lg bg-[var(--color-beige-lighter)] border border-[var(--color-border)] flex items-center justify-center mb-5">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3F8B5F" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    {item.icon}
                  </svg>
                </div>
                <div className="text-[10px] font-bold tracking-[0.12em] uppercase text-[var(--color-primary)] mb-2">{item.time}</div>
                <h3 className="text-lg font-bold text-[var(--color-fg)] mb-2">{item.title}</h3>
      ...
      <section className="bg-[var(--color-bg-card)] py-14 md:py-20 lg:py-28">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          ...
          <div className="max-w-2xl">
            {timeline.map((item, i) => (
              ...
              >
                ...
                <div className="bg-[var(--color-beige-lighter)] border border-[var(--color-border)] rounded-lg h-11 flex items-center justify-center text-[11px] font-bold tracking-[0.04em] text-[var(--color-primary)] px-2 text-center leading-tight">
                  {item.time}
                </div>
                <div className="pt-1">
                  <h4 className="text-base font-bold text-[var(--color-fg)] mb-1.5">{item.title}</h4>
                  <p className="text-sm text-[var(--color-fg-muted)] leading-relaxed mb-2">{item.desc}</p>
                  <span
          ...
          <p className="text-xs text-[var(--color-fg-subtle)] mt-8 italic">
      ...
      {/* ── Markets ── */}
      <section className="bg-[var(--color-grey-section)] py-14 md:py-20 lg:py-28">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {markets.map((m, i) => (
              <motion.div
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.5, delay: i * 0.07 }}
                className="bg-white border border-[var(--color-border)] rounded-xl p-7 shadow-[0_4px_24px_rgba(10,10,10,0.06)]"
              >
                <div className="flex items-center gap-3 mb-3">
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: m.dot }} />
                  <h3 className="text-lg font-bold text-[var(--color-fg)]">{m.title}</h3>
                </div>
                <p className="text-sm text-[var(--color-fg-muted)] leading-relaxed mb-4">{m.desc}</p>
                <div className="flex flex-wrap gap-2">
                  {m.tags.map((tag) => (
                    <span key={tag} className="text-[11px] font-semibold text-[var(--color-fg-muted)] bg-[var(--color-beige-lighter)] border border-[var(--color-border)] px-2.5 py-1 rounded-sm">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {whyJoin.map((item, i) => (
              <motion.div
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.5, delay: i * 0.07 }}
                className="bg-[var(--color-beige-lighter)] border border-[var(--color-border)] rounded-xl p-7 shadow-[0_4px_24px_rgba(10,10,10,0.06)]"
              >
                <div className="text-4xl font-bold text-[var(--color-primary)]/40 mb-3 leading-none">{item.num}</div>
                <h4 className="text-base font-bold text-[var(--color-fg)] mb-2">{item.title}</h4>
                <p className="text-sm text-[var(--color-fg-muted)] leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
      ...
      <section className="bg-white py-14 md:py-20 lg:py-28">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          ...
          <div className="bg-white border border-[var(--color-border)] rounded-2xl p-8 md:p-10 shadow-[0_4px_24px_rgba(10,10,10,0.06)]">
            <h3 className="text-xl md:text-2xl font-bold text-[var(--color-fg)] mb-2">
              How participants move from <span className="text-[var(--color-primary)]">open → paid</span>
            </h3>
            <p className="text-[var(--color-fg-muted)] text-sm leading-relaxed mb-8 max-w-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
              {funnelSteps.map((step, i) => (
                <div
                  key={step.num}
                  className={`relative p-5 text-center ${i < funnelSteps.length - 1 ? 'md:border-r border-[var(--color-border)]' : ''} ${i < funnelSteps.length - 2 ? 'border-b lg:border-b-0 border-[var(--color-border)]' : ''}`}
                >
                  <div className="text-3xl font-bold text-[var(--color-primary)] mb-2">{step.num}</div>
                  <h4 className="text-sm font-semibold text-[var(--color-fg)] mb-2">{step.title}</h4>
                  <p className="text-xs text-[var(--color-fg-muted)] leading-relaxed">{step.desc}</p>
                  {i < funnelSteps.length - 1 && (
                    <span className="hidden md:inline-flex absolute right-[-10px] top-1/2 -translate-y-1/2 w-5 h-5 bg-white border border-[var(--color-border)] rounded-full items-center justify-center text-[var(--color-primary)] text-[10px] z-10">›</span>
                  )}
                </div>
              ))}
            </div>
      ...
      <section className="bg-gradient-to-br from-[var(--color-beige-lighter)] via-[var(--color-beige-light)] to-[var(--color-beige-lighter)] border-y border-[var(--color-primary)]/25 py-16 md:py-20 text-center">
        <div className="max-w-[800px] mx-auto px-4 sm:px-6">
          ...
          <button
            type="button"
            onClick={openChannel}
            className="inline-flex items-center gap-3 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white text-sm md:text-base font-bold tracking-[0.06em] uppercase px-8 py-4 rounded-lg transition-colors cursor-pointer"
          >
