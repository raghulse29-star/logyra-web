...
const coverageCards = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="#3F8B5F" strokeWidth="1.8"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" /></svg>
    ),
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="#3F8B5F" strokeWidth="1.8"><rect x="2" y="3" width="20" height="14" rx="2" /><path d="M8 21h8M12 17v4" /></svg>
    ),
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="#3F8B5F" strokeWidth="1.8"><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg>
    ),
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="#3F8B5F" strokeWidth="1.8"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg>
    ),
  },
];
...
export default function AboutPage() {
  ...
  return (
    <main className="font-sans bg-[var(--color-bg)]">
      ...
      {/* ── Hero ── */}
      <section className="relative bg-[var(--color-bg-section)] pt-28 pb-16 sm:pt-32 sm:pb-20 md:pt-36 md:pb-24 overflow-hidden border-b border-[var(--color-border)]">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-[20%] left-[20%] w-[400px] h-[400px] bg-[var(--color-primary)]/10 blur-[140px] rounded-full" />
          <div className="absolute top-0 right-[10%] w-[320px] h-[320px] bg-[var(--color-primary)]/8 blur-[120px] rounded-full" />
        </div>
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 relative z-10">
          ...
          <motion.h1
            ...
            className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-[var(--color-fg)] leading-[1.06] tracking-tight mb-8 max-w-3xl"
          >
            Built to read<br />
            the market.<br />
          ...
          <motion.p
            ...
            className="text-[var(--color-fg-muted)] text-base md:text-lg max-w-2xl leading-relaxed mb-12"
          >
          ...
          >
            <p className="text-lg md:text-xl lg:text-2xl font-semibold leading-snug text-[var(--color-fg-muted)] tracking-tight">
              <span className="text-[var(--color-fg)] font-bold">Most participants lose not because they lack information</span> — they lose because they have too much of the wrong kind. Unstructured data. Borrowed opinions. Reactive decisions. We were built to fix that. <span className="text-[var(--color-fg)] font-bold">Not with more information. With better thinking.</span>
            </p>
          </motion.blockquote>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="mb-14">
            <div className="flex items-center gap-4 mb-5">
              <span className="text-[var(--color-primary)] text-[10px] font-bold tracking-[0.2em] uppercase whitespace-nowrap">Our Conviction</span>
              <div className="flex-1 h-px bg-[var(--color-border)]" />
            </div>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--color-fg)] mb-4 leading-[1.1]">
              What we<br />
          ...
          <div className="max-w-[860px] border-t border-[var(--color-border)]">
            {beliefs.map((b, i) => (
              <motion.div
                key={b.num}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="grid grid-cols-[40px_1fr] sm:grid-cols-[60px_1fr] gap-5 sm:gap-7 py-8 sm:py-9 border-b border-[var(--color-border)]"
              >
                <div className="text-[13px] font-bold text-[var(--color-fg-subtle)] tracking-[0.08em] pt-1">{b.num}</div>
                <div>
                  <h3 className="text-lg sm:text-xl md:text-[22px] font-bold text-[var(--color-fg)] mb-2.5 leading-snug">{b.title}</h3>
                  <p className="text-[15px] text-[var(--color-fg-muted)] leading-[1.75]">{b.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* ── Numbers ── */}
      <section className="bg-[var(--color-bg-section)] py-16 md:py-24 lg:py-28 border-y border-[var(--color-border)]">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-5">
              <span className="text-[var(--color-primary)] text-[10px] font-bold tracking-[0.2em] uppercase whitespace-nowrap">Track Record</span>
              <div className="flex-1 h-px bg-[var(--color-border)]" />
            </div>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--color-fg)] mb-4 leading-[1.1]">
              Numbers that<br />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 max-w-[900px] border border-[var(--color-border)] rounded-xl overflow-hidden bg-[var(--color-bg-card)]" style={{ boxShadow: '0 4px 24px rgba(10,10,10,0.06)' }}>
            {numbers.map((n, i) => (
              <motion.div
                key={n.label}
                initial={{ opacity: 0, y: 16 }}
                ...
                className={`relative p-8 sm:p-10 group hover:bg-[var(--color-bg-elevated)] transition-colors ${
                  i < 3 ? 'border-b sm:border-b-0 lg:border-b-0 sm:border-r border-[var(--color-border)]' : ''
                } ${i < 2 ? 'sm:border-b lg:border-b-0 border-[var(--color-border)]' : ''}`}
              >
                ...
                <div className="text-[13px] text-[var(--color-fg-muted)] leading-[1.55]">{n.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-16 md:py-24 lg:py-28">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="mb-14">
            <div className="flex items-center gap-4 mb-5">
              <span className="text-[var(--color-primary)] text-[10px] font-bold tracking-[0.2em] uppercase whitespace-nowrap">Research Coverage</span>
              <div className="flex-1 h-px bg-[var(--color-border)]" />
            </div>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--color-fg)] mb-4 leading-[1.1]">
              One framework.<br />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-[900px]">
            {coverageCards.map((c, i) => (
              <motion.div
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.45, delay: i * 0.08 }}
                className="bg-[var(--color-bg-card)] border border-[var(--color-border)] hover:border-[var(--color-primary)]/40 hover:-translate-y-0.5 transition-all duration-300 rounded-xl p-7 sm:p-8"
                style={{ boxShadow: '0 4px 24px rgba(10,10,10,0.06)' }}
              >
                <div className="w-11 h-11 rounded-lg bg-[var(--color-beige-light)] border border-[var(--color-border)] flex items-center justify-center mb-5">
                  <div className="w-5 h-5">{c.icon}</div>
                </div>
                <h3 className="text-lg font-bold text-[var(--color-fg)] mb-2.5">{c.title}</h3>
                <p className="text-sm text-[var(--color-fg-muted)] leading-[1.65]">{c.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* ── Research Philosophy ── */}
      <section className="bg-[var(--color-grey-section)] py-16 md:py-24 lg:py-28 border-y border-[var(--color-border)]">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="mb-14">
            <div className="flex items-center gap-4 mb-5">
              <span className="text-[var(--color-primary)] text-[10px] font-bold tracking-[0.2em] uppercase whitespace-nowrap">Research Philosophy</span>
              <div className="flex-1 h-px bg-[var(--color-border)]" />
            </div>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--color-fg)] mb-4 leading-[1.1]">
              How we<br />
          ...
          <div className="max-w-[820px]">
            <motion.p
              ...
              className="text-xl sm:text-2xl md:text-3xl lg:text-[34px] font-extrabold leading-snug tracking-tight text-[var(--color-fg)] mb-8"
            >
            ...
            <p className="text-base text-[var(--color-fg-muted)] leading-[1.75] mb-6">
            ...
            <p className="text-base text-[var(--color-fg-muted)] leading-[1.75]">
          ...
          {/* Core Pillars */}
          <div className="mt-16">
            <div className="text-[var(--color-primary)] text-[10px] font-bold tracking-[0.2em] uppercase mb-6">Core Pillars</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-[900px]">
              {pillars.map((p, i) => (
                <motion.div
                  ...
                  className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-7"
                  style={{ boxShadow: '0 4px 24px rgba(10,10,10,0.06)' }}
                >
                  ...
                  <p className="text-[13px] text-[var(--color-fg-muted)] leading-[1.65]">{p.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-[var(--color-bg)] py-16 md:py-24 lg:py-28">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-5">
              <span className="text-[var(--color-primary)] text-[10px] font-bold tracking-[0.2em] uppercase whitespace-nowrap">Absolute Clarity</span>
              <div className="flex-1 h-px bg-[var(--color-border)]" />
            </div>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--color-fg)] mb-4 leading-[1.1]">
              What Logyra is.<br />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-[900px]">
            <motion.div
              ...
              viewport={{ once: true, amount: 0.2 }}
              transition={{ duration: 0.5 }}
              className="bg-[var(--color-bg-card)] border border-[var(--color-primary)]/30 rounded-2xl p-7 sm:p-9"
              style={{ boxShadow: '0 18px 48px rgba(10,10,10,0.08)' }}
            >
              ...
              <ul className="space-y-3.5">
                {truthIs.map((item) => (
                  <li key={item} className="flex items-start gap-3 text-sm text-[var(--color-fg-muted)] leading-[1.55]">
            ...
            <motion.div
              ...
              className="bg-[var(--color-bg-card)] border border-[var(--color-error)]/30 rounded-2xl p-7 sm:p-9"
              style={{ boxShadow: '0 18px 48px rgba(10,10,10,0.08)' }}
            >
      ...
      {/* ── CTA Band ── */}
      <section className="bg-gradient-to-br from-[var(--color-beige-lighter)] via-[var(--color-bg-card)] to-[var(--color-beige-light)] border-y border-[var(--color-primary)]/25 py-16 md:py-24 text-center">
        <div className="max-w-[820px] mx-auto px-4 sm:px-6">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--color-fg)] mb-4 leading-[1.1] tracking-tight">
            The research is live.<br />
            <span className="text-[var(--color-primary)]">Access starts free.</span>
          </h2>
          <p className="text-[var(--color-fg-muted)] text-base md:text-lg mb-9 leading-relaxed max-w-[520px] mx-auto">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <button
              type="button"
              onClick={openChannel}
              className="inline-flex items-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white text-sm font-bold tracking-[0.06em] uppercase px-7 py-4 rounded-lg transition-colors cursor-pointer"
            >
              Join Free on Telegram
            </button>
            <Link
              href="/research-desk/inner-circle"
              className="inline-flex items-center gap-2 border border-[var(--color-primary)] text-[var(--color-primary)] hover:bg-[var(--color-primary-tint)] text-sm font-semibold px-6 py-4 rounded-lg transition-colors"
            >
              View Inner Circle →
            </Link>
          </div>
